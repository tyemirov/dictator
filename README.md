...
* `Dockerfile.gpu` now prefetches the default Qwen voice-cloning model, `Qwen/Qwen3-TTS-12Hz-1.7B-Base`, and the Silero Russian `v5_5_ru` preset-speaker model into `/opt/models` during the image build, so the synthesis engines do not need to download weights on first container startup.
...
* The gRPC synthesis service defaults `language_code=ru` requests with no explicit engine to **Silero `v5_5_ru`** at 24 kHz, using preset speaker `baya` unless `preset_speaker` is set to `xenia`; callers can request another positive output sample rate with `audio_format.sample_rate_hz`.
